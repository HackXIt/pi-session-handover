...
export function registerHandoverCommands(pi: ExtensionAPI, state: HandoverRuntimeState, makeId: () => string): void {
	pi.registerCommand("handover", {
		...
		handler: async (args, ctx) => {
			...
			if (command === "auto") {
				...
				ctx.ui.setStatus("pi-session-handover", `handover auto ${autoState.depth}/${autoState.maxDepth}`);
			...
			if (subcommand === "status") {
				...
				if (action === "Cancel") {
					...
					if (auto) {
						...
						ctx.ui.setStatus("pi-session-handover", undefined);
					}
	...
	pi.registerCommand("handover-continue", {
		description: "Internal command used by pi-session-handover to start the replacement session",
		handler: async (args, ctx) => {
			const id = args.trim();
